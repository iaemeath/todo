  ...
  >
    <el-tabs v-model="activeTab" :tab-position="isMobile ? 'top' : 'left'" class="settings-tabs">
      ...
      <el-tab-pane name="view">
        <template #label>
          <span class="tab-label"><el-icon><Monitor /></el-icon> 视觉与外观</span>
        </template>
        ...
        <el-form label-position="top" class="settings-form">
          ...
          <el-card shadow="never" class="setting-card">
            ...
            <el-form-item label="时行数（一个小时划分为几行）">
              <el-select v-model="form.slotDuration" style="width: 100%;">
            ...
            <el-form-item :label="`行高度（单格高度）: ${form.slotHeight}px`">
              <el-slider v-model="form.slotHeight" :min="20" :max="120" :step="2" />
            </el-form-item>
          </el-card>
          ...
          <el-card shadow="never" class="setting-card">
            ...
            <el-form-item :label="`整点分割线粗细: ${form.majorLineWidth}px`">
              <el-slider v-model="form.majorLineWidth" :min="0.5" :max="4" :step="0.5" />
            </el-form-item>
            <el-form-item :label="`整点分割线颜色深度: ${Math.round(form.majorLineOpacity * 100)}%`">
              <el-slider v-model="form.majorLineOpacity" :min="0.05" :max="1" :step="0.05" />
            </el-form-item>
          </el-card>
          ...
          <el-card shadow="never" class="setting-card">
            ...
            <template v-if="form.showMinorLines">
              ...
              <el-form-item :label="`辅助细线粗细: ${form.minorLineWidth}px`">
                <el-slider v-model="form.minorLineWidth" :min="0.5" :max="3" :step="0.5" />
              </el-form-item>
              <el-form-item :label="`辅助细线颜色深度: ${Math.round(form.minorLineOpacity * 100)}%`">
                <el-slider v-model="form.minorLineOpacity" :min="0.05" :max="1" :step="0.05" />
              </el-form-item>
            </template>
          </el-card>
          ...
          <el-card shadow="never" class="setting-card">
            ...
            <el-form-item :label="`线条粗细: ${form.nowIndicatorHeight}px`">
              <el-slider v-model="form.nowIndicatorHeight" :min="1" :max="6" :step="0.5" />
            </el-form-item>
            <el-form-item :label="`透明度: ${Math.round(form.nowIndicatorOpacity * 100)}%`">
              <el-slider v-model="form.nowIndicatorOpacity" :min="0.1" :max="1" :step="0.05" />
            </el-form-item>
          </el-card>
        </el-form>
      </el-tab-pane>
      ...
      <el-tab-pane name="ai">
        <template #label>
          <span class="tab-label"><el-icon><ChatDotRound /></el-icon> AI 助理配置</span>
        </template>
          ...
          <el-card v-if="form.aiMode === 'cloud'" shadow="never" class="setting-card">
            ...
            <el-form label-position="top">
              <el-form-item label="API Base URL">
                <el-input v-model="form.apiBaseUrl" placeholder="例如: https://api.deepseek.com/v1" />
              </el-form-item>
              <el-form-item label="模型名称 (Model)">
                <el-input v-model="form.modelName" placeholder="例如: deepseek-chat 或 gpt-4o-mini" />
              </el-form-item>
      ...
      <el-tab-pane name="usage">
        <template #label>
          <span class="tab-label"><el-icon><DataLine /></el-icon> API 消耗记录</span>
        </template>
...
// Expandable rows in the usage table
...
watch(
  () => [
    ...
    form.value.endHour,
    form.value.nowIndicatorColor,
    form.value.nowIndicatorHeight,
    form.value.nowIndicatorOpacity
  ],
  () => {
    updateSettings({
...
/* Row with label/desc on left, control on right */
.setting-row {
  display: flex;
  align-items: center;
