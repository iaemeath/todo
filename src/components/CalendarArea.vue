...
const calendarOptions = computed(() => {
  ...
  eventDragStart: handleEventDragStart, // 多选整组拖拽：登记其余成员快照
  // 吸附对齐的整组预览：悬停落点变列时其余成员由 FC setDates 同步搬移
  // （api 由闭包注入：composable 不持日历引用）
  eventAllow: eventAllowHandler(() => fullCalendar.value?.getApi() ?? null),
  eventDragStop: () => handleEventDragStop(), // 拖拽结束（含取消）：非 drop 路径复位预览成员
